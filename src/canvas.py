...
class MyCanvas(Canvas):
    ...
    def draw_circle(self, x, y, r=10, color=(255, 0, 0)):
        rx, ry = self.center.x + round(x),self.center.y - round(y)
    ...
    def draw_line(self, p0, p1, color=(0, 0, 0), line_type = "simple", thickness = 2):
        rp0 = self.center.x + round(p0[0]),self.center.y - round(p0[1])
        rp1 = self.center.x + round(p1[0]),self.center.y - round(p1[1])
        if line_type == "simple":
